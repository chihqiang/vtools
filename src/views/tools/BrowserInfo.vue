    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div v-if="loading" class="bg-white rounded-xl shadow-sm p-12">
        <div class="flex flex-col items-center justify-center">
          <svg class="w-12 h-12 text-blue-600 animate-spin mb-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
      ...
      <div v-else-if="!browserInfo" class="bg-white rounded-xl shadow-sm p-12">
        <div class="flex flex-col items-center justify-center">
          ...
          <button @click="getBrowserInfo" class="px-6 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors">
            重新检测
          </button>
        </div>
      </div>
              ...
              <div class="border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.browser.name }} {{ browserInfo.browser.version }}</span>
                  </div>
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-sm text-gray-600 font-medium">内核</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.browser.engine }}</span>
                  </div>
                  <div class="flex justify-between items-start py-2">
                    <span class="text-sm text-gray-600 font-medium">User Agent</span>
                    <div class="flex items-center gap-2">
                      <span class="text-sm text-gray-900 font-mono text-right max-w-[200px] break-all">{{ browserInfo.userAgent }}</span>
                      <button @click="copyToClipboard(browserInfo.userAgent, 'User Agent')" class="p-1.5 bg-gray-100 rounded hover:bg-gray-200 transition-colors">
              ...
              <div class="border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.os.name }} {{ browserInfo.os.version }}</span>
                  </div>
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-sm text-gray-600 font-medium">架构</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.os.architecture }}</span>
                  </div>
                  <div class="flex justify-between items-center py-2">
                    <span class="text-sm text-gray-600 font-medium">平台</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.hardware.platform }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                  ...
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.device.vendor }}</span>
                  </div>
                  <div class="flex justify-between items-center py-2">
                    <span class="text-sm text-gray-600 font-medium">触摸点</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.hardware.maxTouchPoints }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.locale.language }}</span>
                  </div>
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-sm text-gray-600 font-medium">时区</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.locale.timezone }}</span>
                  </div>
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-sm text-gray-600 font-medium">时区偏移</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.locale.timezoneOffset }} 分钟</span>
                  </div>
                  <div class="py-2">
                    <span class="text-sm text-gray-600 font-medium block mb-2">支持语言</span>
                    <div class="flex flex-wrap gap-2 justify-end">
                      <span v-for="(lang, index) in browserInfo.locale.languages" :key="index" class="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-xs">
                        {{ lang }}
                      </span>
                    </div>
                  </div>
              ...
              <div class="border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.hardware.cpuCores }}</span>
                  </div>
                  <div class="flex justify-between items-center py-2">
                    <span class="text-sm text-gray-600 font-medium">内存</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.hardware.memory ? `${browserInfo.hardware.memory} GB` : '未知' }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.screen.width }} × {{ browserInfo.screen.height }}</span>
                  </div>
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-sm text-gray-600 font-medium">可用分辨率</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.screen.availWidth }} × {{ browserInfo.screen.availHeight }}</span>
                  </div>
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-sm text-gray-600 font-medium">颜色深度</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.screen.colorDepth }} 位</span>
                  </div>
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-sm text-gray-600 font-medium">像素深度</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.screen.pixelDepth }} 位</span>
                  </div>
                  <div class="flex justify-between items-center py-2">
                    <span class="text-sm text-gray-600 font-medium">设备像素比</span>
                    <span class="text-sm text-gray-900 font-mono">{{ browserInfo.screen.devicePixelRatio }}</span>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  ...
                  >
                    <svg class="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <path v-if="supported" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
            ...
            <div class="border border-gray-200 rounded-lg overflow-hidden max-w-3xl mx-auto">
              ...
              <div class="p-4 space-y-3">
                <div class="flex justify-between items-center py-2 border-b border-gray-100">
                  ...
                  <span class="text-sm text-gray-900 font-mono">{{ browserInfo.network.connection }}</span>
                </div>
                <div class="flex justify-between items-center py-2 border-b border-gray-100">
                  <span class="text-sm text-gray-600 font-medium">有效类型</span>
                  <span class="text-sm text-gray-900 font-mono">{{ browserInfo.network.effectiveType }}</span>
                </div>
                <div class="flex justify-between items-center py-2 border-b border-gray-100">
                  <span class="text-sm text-gray-600 font-medium">下行速度</span>
                  <span class="text-sm text-gray-900 font-mono">{{ browserInfo.network.downlink ? `${browserInfo.network.downlink} Mbps` : '未知' }}</span>
                </div>
                <div class="flex justify-between items-center py-2">
                  <span class="text-sm text-gray-600 font-medium">往返时间</span>
                  <span class="text-sm text-gray-900 font-mono">{{ browserInfo.network.rtt ? `${browserInfo.network.rtt} ms` : '未知' }}</span>
                </div>
              </div>
            </div>
          </div>
