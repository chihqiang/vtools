...
export function solar2lunar(y: number, m: number, d: number): LunarResult {
  ...
  const shengxiao = Animals[shengxiaoIndex] || ''
  const ganIndex = (year - 4) % 10
  const zhiIndex = (year - 4) % 12
  const ganZhiYear = (Gan[ganIndex] || '') + (Zhi[zhiIndex] || '')
  const lunarMonthStr = (isLeap ? '闰' : '') + (LunarMonth[month - 1] || '') + '月'
  const lunarDayStr = LunarDay[day - 1] || ''
...
export function getGanZhiString(date: Date): string {
  ...
  const yearGanIdx = (lunar.lYear - 4) % 10
  const monthGanBase = [2, 4, 6, 8, 0, 2, 4, 6, 8, 0]
  const monthGanBaseValue = monthGanBase[yearGanIdx] || 0
  const monthGanIdx = (monthGanBaseValue + lunar.lMonth - 1) % 10

  const baseDate = new Date(1900, 0, 31)
  const dayOffset = Math.floor((date.getTime() - baseDate.getTime()) / 86400000)
  const dayGanIdx = (dayOffset + 6) % 10

  const monthZhiIdx = (lunar.lMonth + 1) % 12
  const dayZhiIdx = (dayOffset + 10) % 12

  const ganZhiYear = lunar.ganZhiYear
  const ganZhiMonth = (Gan[monthGanIdx] || '') + (Zhi[monthZhiIdx] || '')
